<template>
  <div>
    ...
    <v-card color="secondary" dark class="pa-0 mt-4">
      <v-card-title class="d-flex justify-space-between">
        <div>
          <p class="font-weight-bold text-h6">{{ subject }}</p>
          <p
            class="
              mt-1
              font-weight-light
        ...
        <v-chip :color="statusChipColor" class="py-2 px-5">{{ status }}</v-chip>
      </v-card-title>
      <v-divider dark class="my-2"></v-divider>
      <v-card-text class="d-flex align-center flex-wrap">
        <p class="my-2 mx-4 text-body-2 white--text">
          <v-icon color="white" size="14" class="mr-2"
            >fas fa-calendar-alt</v-icon
          >
          {{ date }}
        </p>
        <p class="ma-2 text-body-2 white--text">
          <v-icon color="white" size="14" class="mr-2">fas fa-pen-alt</v-icon>
          {{ writer }}
        </p>
      </v-card-text>
      <v-divider dark class="my-2"></v-divider>
      ...
      <v-card-text>
        ...
        >
          ...
          <div class="border-dark pa-3">
            <div>
              <p class="text-body-2 font-weight-light white--text">
                {{ message.msg }}
              </p>
              <a
                v-if="message.file"
              ...
              <p class="mt-4 text-caption grey--text text--lighten-2">
